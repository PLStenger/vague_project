...
RAREFACTION_DEPTH_EUK=2   # exploration initiale
...
#
# IMPORTANT :
#   - on laisse table-decontam.qza comme table procaryotes + eucaryotes
#     pour l’analyse globale
#   - on construit une table eucaryotes PR2-friendly en EXCLUANT Bacteria/Archaea
#   - pas de --p-include du tout → évite les tables vides
# =============================================================================
log "Construction table candidats eucaryotes (exclusion Bacteria/Archaea)"

# Table euk candidates : tout ce qui N'EST PAS Bacteria ni Archaea,
# sans supprimer les échantillons vides (KNS à faible signal)
...
# Séquences euk candidates correspondantes
...
conda run -n "$QIIME2_ENV" biom convert \
    ...
    --to-tsv
log "Échantillons dans table-euk-candidates (doit inclure PA et KNS) :"
awk 'NR==2 {for(i=2;i<=NF;i++) print $i}' "${TMPDIR}/euk_check/table.tsv" | sort

# Pour les procaryotes : on réutilise table-decontam.qza directement
...
# =============================================================================
# 07e — Barplot procaryotes (SILVA) à partir de table-prokaryotes
# =============================================================================
